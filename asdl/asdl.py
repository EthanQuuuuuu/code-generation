...
class ASDLGrammar(object):
    """
    Collections of types, constructors and productions
    """
    ...
    def from_text(text):
        ...
        #Last three lines read form the asdl_file and return a list of productions template | primitive type names
        #primitive type names contained in the first element of line, i.e. lines[0]
        line_no = 0
...
class Field(object):
    ...
    def __repr__(self, plain = False):
        plain_repr = '%s%s %s' % (self.type.__repr__(plain = True),
                                  Field.get_cardinality_repr(self.cardinality),self.name)
