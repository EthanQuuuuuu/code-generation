...
class Hypothesis(object):
    ...
    def apply_action(self, action):
        if self.tree is None:
            assert isinstance(action, ApplyRuleAction),  'Invalid action [%s], only ApplyRule action is valud at the beginning of decoding' 
            ...
            if isinstance(self.frontier_field.type, ASDLCompositeType):
                if isinstance(action, ApplyRuleAction):
                    ...
                    assert self.frontier_field.cardinality in ('optimal','multiple'), 'Reduce action can only be applied on field with multiple cardinality'
                ...
                if isinstance(action, GenTokenAction):
                    ...
                    assert self.frontier_field.cardinality in ('optional','multiple'), 'Reduce Action can only be applied to field with multiple cardinality'
                    self.frontier_field.set_finish()
                    self.update_frontier_info()
                else:
