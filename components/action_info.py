...
class ActionInfo(object):
    """
    Sufficient statistics for making a prediction of an ation at a time step
    """
    ...
    def __repr__(self, verbose = False):
        repr_str = '%s (t=%d, p_t=%d, frontier_field=%s' % (repr(self.action), self.t, self.parent_t, self.frontier_field.__repr__(True) if self.frontier_field else 'None')
        return repr_str
