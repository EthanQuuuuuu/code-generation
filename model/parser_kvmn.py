...
# from model.tfmencoder import TransformerEncoder
...
#         self.production_key_embed = nn.Embedding(len(transition_system.grammar) + 1, args.action_embed_size)
#         # self.production_value_embed = nn.Embedding(len(transition_system.grammar) + 1, args.action_embed_size)
...
#         self.encoder_transformer = TransformerEncoder(len(vocab.source), args.embed_size*2, args.embed_size*2, 
#                                                       args.embed_size*2,args.embed_size*2,[args.embed_size*2],args.embed_size*2, 
#                                                       args.embed_size*4, 8, 3, args.dropout)
...
#             input_dim += args.att_vec_size * (not args.no_input_feed)  # input feeding dimension feed into decoder
...
#             self.tgt_token_readout = lambda q: F.linear(self.read_out_act(q),
#                                                         self.primitive_embed.weight, self.tgt_token_readout_b)
#         self.production_linear = nn.Linear(args.att_vec_size, len(transition_system.grammar) + 1)
#         # dropout layer
...
#     def apply_kvmn(self, query_vectors, key_embedding, value_emebedding):
#         """
#         Args:
#             query_vectors: (tgt_action_len, batch_size, hidden_size)
#             key_embedding, value_emebdding: (grammar_len, hidden_size)
#         Return:
#             query_vectors_with_apply_rule_memory: (tgt_action_len, batch_size, hidden_size)
#         """
#         apply_prob = torch.matmul(query_vectors, key_embedding.weight.T) # (tgt_action_len, batch_size, grammar_len)
#         apply_prob = F.softmax(apply_prob,dim = -1) # (tgt_action_len, batch_size, grammar_len)
#         u = torch.matmul(apply_prob, value_emebedding.weight)
#         return query_vectors + u
...
#         enc_output = self.encoder_transformer(src_sents_var.permute(1,0),torch.cuda.LongTensor(src_sents_len))

#         return src_encodings, (last_state, last_cell), enc_output
...
#         src_encodings, (last_state, last_cell), enc_output = self.encode(batch.src_sents_var, batch.src_sents_len) #Last-cell is the memory cell
#         dec_init_vec = self.init_decoder_state(last_state, last_cell)
#         # query vectors are sufficient statistics used to compute action probabilities
#         # query_vectors: (tgt_action_len, batch_size, hidden_size)

#         # if use supervised attention return size (batch.max_tgt_actions, Batch_size, embedding_size)
#         if self.args.sup_attention:
#             query_vectors, att_prob = self.decode(batch, src_encodings, dec_init_vec)
#         else:
#             # query_vectors = self.decode(batch, src_encodings, dec_init_vec) original one
#             query_vectors = self.decode(batch, enc_output, dec_init_vec)
#         '''
#         batch.xxxx_matix are ground truths
#         '''
#         # ApplyRule (i.e., ApplyConstructor) action probabilities
#         # (tgt_action_len, batch_size, grammar_size)
#         # self.production_readout(query_vectors) of size (batch.max_tgt_actions, batch_size, num_apply_rule)
#         mem_production = self.apply_kvmn(query_vectors, self.production_key_embed, self.production_embed)

#         # apply_rule_prob = F.softmax(self.production_readout(query_vectors), dim=-1) # (batch.max_tgt_actions, batch_size, num_apply_rule)
#         apply_rule_prob_mem = F.softmax(self.production_linear(mem_production), dim=-1) # (batch.max_tgt_actions, batch_size, num_apply_rule)
#         # probabilities of target (gold-standard) ApplyRule actions
#         # (tgt_action_len, batch_size)
#         tgt_apply_rule_prob = torch.gather(apply_rule_prob_mem, dim=2,
...
#         '''
#         dot_prod_attention uses current hidden state and src_enc to apply an attention
#         with outputting a context vector which is a weighted sum of src_enc for each batch
#         '''
...
#             if t == 0:
#                 '''
#                 When t == 0, initialize a (Batch_size, 704[Action; ctx; frontier_field; ???]) zero tensor to x
#                 Afterwards, x comes from the input to decoder
#                 '''
...
#             else:
#                 '''
#                 This part embed the current actions
#                 '''
#                 a_tm1_embeds = []
...
#                             a_tm1_embed = self.production_embed.weight[self.grammar.prod2id[a_tm1.action.production]] #SelectBackward
#                             #a_tm1_embed = self.production_embed(torch.cuda.LongTensor([self.grammar.prod2id[a_tm1.action.production]])) EmbeddingBackward
...
#                 '''
#                 This part concat the actions embedding with last context enbedding and frontier field embedding
#                 '''
#                 inputs = [a_tm1_embeds]
...
#                 '''
#                 This part concat the parent node embedding of hidden_state
#                 '''
...
#             '''
#             From t == 0 using the initial h_tm1 and x and the output of encoder to update (h_t, c_t) which
#             is next h_tm1, att_t which is ctx_vector, and att_weight
#             '''
...
#         src_encodings, (last_state, last_cell), enc_output = self.encode(src_sent_var, [len(src_sent)])
#         # (1, src_sent_len, hidden_size)
#         # src_encodings_att_linear = self.att_src_linear(src_encodings)
#         src_encodings_att_linear = self.att_src_linear(enc_output)
...
#             exp_src_encodings = enc_output.expand(hyp_num, enc_output.size(1), enc_output.size(2))
...
#             apply_att = self.apply_kvmn(att_t,self.production_key_embed,self.production_embed)
#             apply_rule_log_prob = F.log_softmax(self.production_linear(apply_att), dim=-1)
