...
# from components import action_info,vocab,dataset
...
class Django(object):
    ...
    def parse_django_dataset(annot_file, code_file, asdl_file_path, max_query_len=70, vocab_freq_cutoff=10):
        ...
        for idx, (src_query, tgt_code) in enumerate(zip(open(annot_file), open(code_file))):
            ...
            if type(python_ast).__name__ == 'Call':
                import pdb; pdb.set_trace()
    ...
    def process_django_dataset():
        ...
        pickle.dump(train, open('data/django/train_test.bin', 'w'))
        pickle.dump(dev, open('data/django/dev_test.bin', 'w'))
        pickle.dump(test, open('data/django/test_test.bin', 'w'))
        pickle.dump(vocab, open('data/django/vocab.freq%d.test.bin' % vocab_freq_cutoff, 'w'))
