...
def init_arg_parser():
    ...
    arg_parser.add_argument('--seed', default=1218, type=int, help='Random seed')
    ...
    arg_parser.add_argument('--asdl_file', default = 'asdl/lang/py3/py3_asdl.simplified.txt',type=str, help='Path to ASDL grammar specification')
    # arg_parser.add_argument('--mode', type = str, defualt = 'train')
    arg_parser.add_argument('--mode', choices=['train', 'test', 'interactive',
                                               'train_paraphrase_identifier', 'train_reconstructor','rerank'], default = 'train', help='Run mode')

    #### Modularized configuration ####
    arg_parser.add_argument('--parser', type=str, default='default_parser', required=False, help='name of parser class to load')
    arg_parser.add_argument('--transition_system', type=str, default='python3', required=False, help='name of transition system to use')
    arg_parser.add_argument('--evaluator', type=str, default='conala_evaluator', required=False, help='name of evaluator class to use')
    ...
    #### Training ####
    arg_parser.add_argument('--vocab', type=str, default = 'data/conala/vocab.src_freq3.code_freq3.bin', help='Path of the serialized vocabulary')
    arg_parser.add_argument('--glove_embed_path', default=None, type=str, help='Path to pretrained Glove mebedding')

    arg_parser.add_argument('--train_file', type=str,default='data/conala/train.all_0.bin', help='path to the training target file')
    arg_parser.add_argument('--dev_file', type=str, default='data/conala/dev.bin',help='path to the dev source file')
    ...
    arg_parser.add_argument('--save_to', default='save_models/conala/junefourteenth', type=str, help='Save trained model to')
    ...
    arg_parser.add_argument('--glorot_init', default=True, action='store_true', help='Use glorot initialization')
    arg_parser.add_argument('--clip_grad', default=5., type=float, help='Clip gradients')
    arg_parser.add_argument('--max_epoch', default=80, type=int, help='Maximum number of training epoches')
    ...
    arg_parser.add_argument('--lr_decay_after_epoch', default=15, type=int, help='Decay learning rate after x epoch')
    ...
    arg_parser.add_argument('--load_model', default='save_models/django/june15.bin', type=str, help='Load a pre-trained model')
    arg_parser.add_argument('--beam_size', default=15, type=int, help='Beam size for beam search')
    ...
    arg_parser.add_argument('--save_decode_to', default='decodes/django/juenseventh.test.decode', type=str, help='Save decoding results to file')
...
def init_arg_parser_django_test():
    arg_parser = argparse.ArgumentParser()

    #### General configuration ####
    arg_parser.add_argument('--seed', default=2260, type=int, help='Random seed')
    arg_parser.add_argument('--cuda', action='store_true', default=True, help='Use gpu')
    arg_parser.add_argument('--lang', choices=['python', 'lambda_dcs', 'wikisql', 'prolog', 'python3'], default='python',
                            help='[Deprecated] language to parse. Deprecated, use --transition_system and --parser instead')
    arg_parser.add_argument('--asdl_file', default = 'asdl/lang/py/py_asdl.txt',type=str, help='Path to ASDL grammar specification')
    # arg_parser.add_argument('--mode', type = str, defualt = 'train')
    arg_parser.add_argument('--mode', choices=['train', 'test', 'interactive',
                                               'train_paraphrase_identifier', 'train_reconstructor','rerank'], default = 'train', help='Run mode')
    ...
    # parent information switch for decoder LSTM
    arg_parser.add_argument('--no_parent_production_embed', default=True, action='store_true',
                            help='Do not use embedding of parent ASDL production to update decoder LSTM state')
    arg_parser.add_argument('--no_parent_field_embed', default=False, action='store_true',
                            help='Do not use embedding of parent field to update decoder LSTM state')
    arg_parser.add_argument('--no_parent_field_type_embed', default=True, action='store_true',
                            help='Do not use embedding of the ASDL type of parent field to update decoder LSTM state')
    arg_parser.add_argument('--no_parent_state', default=False, action='store_true',
                            help='Do not use the parent hidden state to update decoder LSTM state')
    ...
    #### Training ####
    arg_parser.add_argument('--vocab', type=str, default = "data/django/vocab.freq15.bin", help='Path of the serialized vocabulary')
    arg_parser.add_argument('--glove_embed_path', default=None, type=str, help='Path to pretrained Glove mebedding')

    arg_parser.add_argument('--train_file', type=str,default='data/django/train.bin', help='path to the training target file')
    arg_parser.add_argument('--dev_file', type=str, default='data/django/dev.bin',help='path to the dev source file')
    arg_parser.add_argument('--pretrain', type=str,help='path to the pretrained model file')
    ...
    arg_parser.add_argument('--save_to', default='save_models/django/july5', type=str, help='Save trained model to')
    arg_parser.add_argument('--save_all_models', default=False, action='store_true', help='Save all intermediate checkpoints')
    arg_parser.add_argument('--patience', default=5, type=int, help='Training patience')
    arg_parser.add_argument('--max_num_trial', default=5, type=int, help='Stop training after x number of trials')
    arg_parser.add_argument('--uniform_init', default=None, type=float,
    ...
    #### decoding/validation/testing ####
    arg_parser.add_argument('--load_model', default='save_models/django/june17.bin', type=str, help='Load a pre-trained model')
    arg_parser.add_argument('--beam_size', default=15, type=int, help='Beam size for beam search')
    arg_parser.add_argument('--decode_max_time_step', default=100, type=int, help='Maximum number of time steps used '
                                                                                  'in decoding and sampling')
    arg_parser.add_argument('--sample_size', default=5, type=int, help='Sample size')
    arg_parser.add_argument('--test_file', default = "data/django/test.bin",type=str, help='Path to the test file')
    arg_parser.add_argument('--save_decode_to', default='decodes/django/july5.test.decode', type=str, help='Save decoding results to file')
...
def init_arg_parser_atis_test():
    arg_parser = argparse.ArgumentParser()

    #### General configuration ####
    arg_parser.add_argument('--seed', default=1370, type=int, help='Random seed')
    arg_parser.add_argument('--cuda', action='store_true', default=True, help='Use gpu')
    arg_parser.add_argument('--lang', choices=['python', 'lambda_dcs', 'wikisql', 'prolog', 'python3'], default='lambda_dcs',
                            help='[Deprecated] language to parse. Deprecated, use --transition_system and --parser instead')
    arg_parser.add_argument('--asdl_file', default = 'asdl/lang/lambda_dcs/lambda_asdl.txt',type=str, help='Path to ASDL grammar specification')
    # arg_parser.add_argument('--mode', type = str, defualt = 'train')
    arg_parser.add_argument('--mode', choices=['train', 'test', 'interactive',
                                               'train_paraphrase_identifier', 'train_reconstructor','rerank'], default = 'train', help='Run mode')
    ...
    #### Modularized configuration ####
    arg_parser.add_argument('--parser', type=str, default='default_parser', required=False, help='name of parser class to load')
    arg_parser.add_argument('--transition_system', type=str, default='lambda_dcs', required=False, help='name of transition system to use')
    arg_parser.add_argument('--evaluator', type=str, default='default_evaluator', required=False, help='name of evaluator class to use')
    ...
    # Embedding sizes
    arg_parser.add_argument('--embed_size', default=128, type=int, help='Size of word embeddings')
    arg_parser.add_argument('--action_embed_size', default=256, type=int, help='Size of ApplyRule/GenToken action embeddings')
    arg_parser.add_argument('--field_embed_size', default=32, type=int, help='Embedding size of ASDL fields')
    arg_parser.add_argument('--type_embed_size', default=32, type=int, help='Embeddings ASDL types')
    ...
    arg_parser.add_argument('--no_input_feed', default=False, action='store_true', help='Do not use input feeding in decoder LSTM')
    arg_parser.add_argument('--no_copy', default=True, action='store_true', help='Do not use copy mechanism')
    ...
    #### Training ####
    arg_parser.add_argument('--vocab', type=str, default = "data/atis/vocab.freq2.bin", help='Path of the serialized vocabulary')
    arg_parser.add_argument('--glove_embed_path', default=None, type=str, help='Path to pretrained Glove mebedding')

    arg_parser.add_argument('--train_file', type=str,default='data/atis/train.bin', help='path to the training target file')
    arg_parser.add_argument('--dev_file', type=str, default='data/atis/dev.bin',help='path to the dev source file')
    arg_parser.add_argument('--pretrain', type=str,help='path to the pretrained model file')
    ...
    arg_parser.add_argument('--save_to', default='save_models/atis/june21', type=str, help='Save trained model to')
    arg_parser.add_argument('--save_all_models', default=False, action='store_true', help='Save all intermediate checkpoints')
    arg_parser.add_argument('--patience', default=5, type=int, help='Training patience')
    arg_parser.add_argument('--max_num_trial', default=5, type=int, help='Stop training after x number of trials')
    arg_parser.add_argument('--uniform_init', default=None, type=float,
    ...
    #### decoding/validation/testing ####
    arg_parser.add_argument('--load_model', default='save_models/atis/june21.bin', type=str, help='Load a pre-trained model')
    arg_parser.add_argument('--beam_size', default=5, type=int, help='Beam size for beam search')
    arg_parser.add_argument('--decode_max_time_step', default=110, type=int, help='Maximum number of time steps used '
                                                                                  'in decoding and sampling')
    arg_parser.add_argument('--sample_size', default=5, type=int, help='Sample size')
    arg_parser.add_argument('--test_file', default = "data/atis/test.bin",type=str, help='Path to the test file')
    arg_parser.add_argument('--save_decode_to', default='decodes/atis/june17.test.decode', type=str, help='Save decoding results to file')
...
def init_arg_parser_geo_test():
    ...
    arg_parser.add_argument('--no_parent_production_embed', default=True, action='store_true',
                            help='Do not use embedding of parent ASDL production to update decoder LSTM state')
    arg_parser.add_argument('--no_parent_field_embed', default=True, action='store_true',
                            help='Do not use embedding of parent field to update decoder LSTM state')
    arg_parser.add_argument('--no_parent_field_type_embed', default=True, action='store_true',
                            help='Do not use embedding of the ASDL type of parent field to update decoder LSTM state')
    arg_parser.add_argument('--no_parent_state', default=True, action='store_true',
                            help='Do not use the parent hidden state to update decoder LSTM state')
    ...
    #### Training ####
    arg_parser.add_argument('--vocab', type=str, default = "data/geo/vocab.freq2.bin", help='Path of the serialized vocabulary')
    arg_parser.add_argument('--glove_embed_path', default=None, type=str, help='Path to pretrained Glove mebedding')

    arg_parser.add_argument('--train_file', type=str,default='data/geo/train.bin', help='path to the training target file')
    arg_parser.add_argument('--dev_file', type=str,default = 'data/geo/test.bin', help='path to the dev source file')
    arg_parser.add_argument('--pretrain', type=str,help='path to the pretrained model file')
    ...
    arg_parser.add_argument('--save_to', default='save_models/geo/june23.5', type=str, help='Save trained model to')
    arg_parser.add_argument('--save_all_models', default=False, action='store_true', help='Save all intermediate checkpoints')
    arg_parser.add_argument('--patience', default=1000, type=int, help='Training patience')
    arg_parser.add_argument('--max_num_trial', default=5, type=int, help='Stop training after x number of trials')
    arg_parser.add_argument('--uniform_init', default=None, type=float,
                            help='If specified, use uniform initialization for all parameters')
    arg_parser.add_argument('--glorot_init', default=True, action='store_true', help='Use glorot initialization')
    arg_parser.add_argument('--clip_grad', default=5., type=float, help='Clip gradients')
    arg_parser.add_argument('--max_epoch', default=200, type=int, help='Maximum number of training epoches')
    arg_parser.add_argument('--optimizer', default='Adam', type=str, help='optimizer')
    arg_parser.add_argument('--lr', default=0.0025, type=float, help='Learning rate')
    arg_parser.add_argument('--lr_decay', default=0.985, type=float,
                            help='decay learning rate if the validation performance drops')
    arg_parser.add_argument('--lr_decay_after_epoch', default=20, type=int, help='Decay learning rate after x epoch')
    arg_parser.add_argument('--decay_lr_every_epoch', action='store_true', default=True, help='force to decay learning rate after each epoch')
    arg_parser.add_argument('--reset_optimizer', action='store_true', default=False, help='Whether to reset optimizer when loading the best checkpoint')
    arg_parser.add_argument('--verbose', action='store_true', default=False, help='Verbose mode')
    arg_parser.add_argument('--eval_top_pred_only', action='store_true', default=False,
                            help='Only evaluate the top prediction in validation')
    ...
    #### decoding/validation/testing ####
    arg_parser.add_argument('--load_model', default='save_models/geo/june23.4.bin', type=str, help='Load a pre-trained model')
    arg_parser.add_argument('--beam_size', default=5, type=int, help='Beam size for beam search')
    arg_parser.add_argument('--decode_max_time_step', default=110, type=int, help='Maximum number of time steps used '
                                                                                  'in decoding and sampling')
    arg_parser.add_argument('--sample_size', default=5, type=int, help='Sample size')
    arg_parser.add_argument('--test_file', default = "data/geo/test.bin",type=str, help='Path to the test file')
    arg_parser.add_argument('--save_decode_to', default='decodes/atis/june17.test.decode', type=str, help='Save decoding results to file')
