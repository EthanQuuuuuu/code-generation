...
import sys
print(sys.path)
# sys.path.remove('/cm/shared/apps/ml-pythondeps-py37-cuda10.2-gcc/4.3.3/lib/python3.7/site-packages/')
# sys.path.remove('//data/Qs/anaconda3/envs/tranx/lib/python3.7/site-packages')
# sys.path.append('//data/Qs/anaconda3/envs/tranx/lib/python3.7/site-packages')
...
from common.utils import update_args, init_arg_parser_django_test,init_arg_parser_atis_test,init_arg_parser_geo_test
...
def train(args):
    ...
    transition_system = Registrable.by_name(args.transition_system)(grammar) #python3
    parser_cls = Registrable.by_name(args.parser)  # TODO: add arg default_parser
    if args.pretrain:
    ...
    while True:
        ...
        for batch_examples in tqdm(train_set.batch_iter(batch_size=args.batch_size, shuffle=True)):
            batch_examples = [e for e in batch_examples if len(e.tgt_actions) <= args.decode_max_time_step]
            train_iter += 1
            optimizer.zero_grad()
            # until here still not convert tokens to ids
            ...
            sample_length += len(batch_examples)
        ...
        print(f'Epoch: {epoch}, Loss: {log_loss/sample_length}')
        ...
        if args.dev_file:
            if epoch % args.valid_every_epoch == 0:
                print('[Epoch %d] begin validation' % epoch, file=sys.stdout)
                ...
                print('[Epoch %d] evaluate details: %s, dev %s: %.5f (took %ds)' % (
                                    ...
                                    dev_score,
                                    time.time() - eval_start), file=sys.stdout)
        ...
        if epoch == args.max_epoch:
            print('reached max epoch, stop!', file=sys.stdout)
            exit(0)
...
def train_rerank_feature(args):
    ...
    print('begin training decoder, %d training examples, %d dev examples' % (len(train_set), len(dev_set)), file=sys.stdout)
    print('vocab: %s' % repr(vocab), file=sys.stdout)
    ...
    while True:
        ...
        print('[Epoch %d] dev_score=%.5f took %ds' % (epoch, dev_acc, time.time() - eval_start), file=sys.stdout)
        ...
        if is_better:
            patience = 0
            model_file = args.save_to + '.bin'
            print('save currently the best model ..', file=sys.stdout)
            print('save model to [%s]' % model_file, file=sys.stdout)
            model.save(model_file)
            # also save the optimizers' state
            torch.save(optimizer.state_dict(), args.save_to + '.optim.bin')
...
def test(args):
    ...
    print('load model from [%s]' % args.load_model, file=sys.stdout)
    ...
    print(eval_results, file=sys.stdout)
...
if __name__ == '__main__':
    arg_parser = init_arg_parser_django_test()
    args = init_config()
